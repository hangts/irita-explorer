            ...
            >
              ...
              >
                ...
                <p class="home_tx_time_content">
                  <span class="home_tx"
                    >{{ $t('ExplorerLang.home.txn') }}: <span>{{ item.txNumber }}</span></span
                  >
              ...
              <p class="home_transaction_time_content">
                <span class="home_transaction">
                  ...
                  <el-tooltip effect="dark" :content="item.hash" placement="top">
                    <router-link :to="`/tx?txHash=${item.hash}`">{{
                      `${item.hash.substr(0, hashLength)}...`
                    }}</router-link>
                  </el-tooltip>
                </span>
                <span class="home_age_time">
  ...
  methods: {
    ...
    setTipDisplay(txType) {
      ...
      if (txType && Array.isArray(txType)) {
        ...
        const data = txType.reduce(function (prev, next) {
      ...
      return (
        this.TX_TYPE_DISPLAY[txType?.toString()] ||
        this.TX_TYPE_DISPLAY[txType] ||
        txType?.toString() ||
        txType
      );
    },
    async getLastBlocks() {
      try {
        const blockData = await getBlockList(1, 10, false);
        if (blockData && blockData.data && blockData.data.length) {
          ...
          this.latestBlockArray = blocks.map((item) => {
            return {
              flShowTranslationalAnimation: item.flShowTranslationalAnimation
      ...
      try {
        ...
        if (res) {
          ...
          this.latestTransaction = res.data.map((item) => {
            return {
              flShowTranslationalAnimtation: item.flShowTranslationalAnimation
                ? item.flShowTranslationalAnimation
                : '',
              showAnimation: item.showAnimation ? item.showAnimation : '',
              hash: item.tx_hash,
    ...
    showBlockFadeinAnimation(blockList) {
      const storedLastBlockHeight = sessionStorage.getItem('lastBlockHeight')
    ...
    componentAgeTime(beginTime, endTime) {
      return (
        (Number(new Date(beginTime).getTime()) - Number(new Date(endTime).getTime())) /
        1000 /
        100
      ).toFixed(2);
    },
    ...
    async getProposalsByStatus() {
      try {
        const res = await getProposalsListApi(
