    ...
    <p v-for="(info, key) in detailInfo" :key="key" v-if="info.label">
      ...
      <template v-else-if="info.isAddress">
        ...
        <template v-else>
          ...
          <span v-else>
            <router-link class="address_link" :to="`/address/${info.value}`">{{
              info.value
            }}</router-link>
          </span>
        </template>
      ...
      <template v-else-if="info.isSchema">
        <LargeString
          :isShowPre="Tools.isJSON(info.value)"
          v-if="info.value"
          :text="info.value"
      ...
      <template v-else-if="info.isUri">
        <div class="wrap" v-if="info.value && info.value !== '[do-not-modify]'">
          <a class="text" v-if="Tools.testUrl(info.value)" :href="info.value" target="_blank">{{
            info.value
          }}</a>
          <a
            class="text"
            v-else-if="startStr(info.value)"
            :href="'http://' + info.value"
            target="_blank"
            >{{ info.value }}</a
          >
      ...
      <template v-else-if="info.isComplexAddr">
        ...
        <span
          v-else
          @click="addressRoute(info.value)"
          :class="
            info.value.startsWith(COSMOS_ADDRESS_PREFIX) ||
            info.value.startsWith(IRIS_ADDRESS_PREFIX)
              ? 'address_link'
              : ''
          "
          >{{ info.value }}</span
      ...
      <template v-else-if="info.isMtLink">
        <span class="verticalShow">
          <router-link :to="`/mt/token?denom=${getMtDenomId(detailInfo)}&tokenId=${info.value}`">{{
              info.value
            }}</router-link>
        </span>
      </template>
      ...
      <template v-else>
        <span
          :style="{ color: info.value === $t('ExplorerLang.common.failed') ? '#fa8593' : '' }"
          >{{ formatValue(info.value) }}</span
        >
      </template>
...
export default {
  ...
  methods: {
    getMtDenomId(info) {
      const mtDenomId = info.find(
        (item) => item.label === this.$t('ExplorerLang.transactionInformation.mintMt.mtDenomId')
      );
      return mtDenomId?.value || '';
    },
    judgeCosmos(addr) {
